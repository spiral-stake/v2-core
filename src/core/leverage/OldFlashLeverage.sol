...
import {LevPosition} from "../structs/LeveragePosition.sol";
 ...
 */
contract OldFlashLeverage is IERC3156FlashBorrower, TokenHelper, Ownable2Step {
    ...
    mapping(address user => LevPosition[]) private s_userLeveragePositions;
    ...
    function unleverage(uint256 leveragePositionId) external {
        LevPosition memory leveragePosition = s_userLeveragePositions[
            msg.sender
        ][leveragePositionId];
    ...
    function getUserLeveragePositions(
        address user
    ) external view returns (LevPosition[] memory) {
